...
public class MultipartHttpServletRequestFileUploadProcessor implements
		FileUploadProcessor {

	private static final Logger logger = LoggerFactory
			.getLogger(MultipartHttpServletRequestFileUploadProcessor.class);
	...
	public LinkedList<FileMeta> processUploadedFile(HttpServletRequest request,
		...
		MultipartHttpServletRequest mhttprequest = (MultipartHttpServletRequest) request;
		
		// 1. build an iterator
		Iterator<String> mitr = mhttprequest.getFileNames();
		...
		while (mitr.hasNext()) {
			...
			MultipartFile item = mhttprequest.getFile(mitr.next());
			logger.debug("Uploading file: " + item.getOriginalFilename());
			...
			if (Utils.isValidFileType(item.getContentType())) {
				if (item.getSize() <= fileSizeAllowed) {
					...
					fileMeta.setFileName(item.getOriginalFilename());
				...
				_fileUploadStatusMessage = item.getOriginalFilename()
